...
def accuracyScore(documentsLenght, locationRetrieve):
    locationReal = {}
    dataReal = open('evaluation/locationReal.txt', 'r')
    for i in dataReal:
        data = i.split(':')
        key = data[0]
        lokasi = data[1].strip('\n').split(', ')
        locationReal[key] = lokasi
    ...
    sumPrecisionDoc = 0
    sumFScoreDoc = 0
    for key in locationRetrieve: 
        lenTrueRetDoc = 0
        if key in locationReal:
            for x in locationRetrieve[key]:
                for y in locationReal[key]:
                    if x == y:
                        lenTrueRetDoc += 1
                        del y
                        break
                        ...
                        pass
            lenTrueRealDoc = len(locationReal[key])
            lenRetrieveDoc = len(locationRetrieve[key])

        Recall = lenTrueRetDoc/lenTrueRealDoc
        Precision = lenTrueRetDoc/lenRetrieveDoc
        FScore = (Recall * Precision)/ (0.5 * (Recall + Precision))

        RecallDoc[key] = Recall
        PrecisionDoc[key] = Precision
        FScoreDoc[key] = FScore

        sumRecallDoc += RecallDoc[key]
        sumPrecisionDoc += PrecisionDoc[key]
        sumFScoreDoc += FScoreDoc[key]

    print('\nBanyak Semua Dokumen: ',documentsLenght)
    print('Final Recall: ',sumRecallDoc/documentsLenght)
    print('Final Precision: ',sumPrecisionDoc/documentsLenght)
    print('Final FScore: ',sumFScoreDoc/documentsLenght)

def dataTraining(): #ini kalo misalnya pake data training
    documentsText = listDocuments()
    documentsLenght = len(documentsText)
    locationRetrieve = {}
    for index in documentsText:
        textPrep = documentsText[index]
        locations = []
        locations = ruleBased(textPrep, locations)
        locationRetrieve[index] = locations
    
    # showLocations(locationRetrieve)
    # writeToFile(locationRetrieve)
    accuracyScore(documentsLenght, locationRetrieve)
